...
int adventurer_function(struct gameState *state)
{
  ...
  while (drawntreasure < 2)
  {
    ...
    { //if the deck is empty we need to shuffle discard and add to deck **BUG: changed <1 to ==1**
...
//remodel
int remodel_function(struct gameState *state, int choice1, int choice2, int handPos)
{
  int j = 0;
  int i = 0;
  int currentPlayer = whoseTurn(state);
